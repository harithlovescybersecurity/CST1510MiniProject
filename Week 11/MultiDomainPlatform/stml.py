...
def init_db():
    ...
    # Insert sample incidents if empty
    cursor.execute("SELECT COUNT(*) FROM cyber_incidents")
    if cursor.fetchone()[0] == 0:
        sample_incidents = [
            ("2024-11-01 09:30:00", "Phishing", "High", "Open", "Suspicious email campaign", "admin"),
            ("2024-11-02 14:15:00", "Malware", "Critical", "Investigating", "Ransomware detected", "admin"),
            ("2024-11-03 11:00:00", "Data Breach", "Medium", "Resolved", "Unauthorized access", "admin"),
            ("2024-11-04 16:45:00", "DDoS", "High", "Open", "Distributed denial of service", "admin"),
            ("2024-11-05 10:20:00", "Phishing", "Low", "Resolved", "Fake invoice emails", "admin")
        ]
        ...
        print("Inserted sample incidents into database")
    conn.commit()
    conn.close()
            ...
            with st.form("add_incident"):
                ...
                if st.form_submit_button("+Add Incident"):
                    ...
                    if success:
                        ...
                        st.rerun()  # Added rerun to refresh
                    else:
                        st.error(result)
            ...
            if len(users_data) > 0:
                ...
                with col1:
                    ...
                    with st.form("update_role_form"):
                        ...
                        new_role = st.selectbox("New Role", ["user", "analyst", "admin"],index=["user", "analyst", "admin"].index(current_role),key="role_select")
